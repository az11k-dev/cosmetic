...
import { VendorImage } from "@/types/data.types";
...
const VendorUpload = () => {
  ...
  const [imagePreview, setImagePreview] = useState<VendorImage>({
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        if (event.target && typeof event.target.result === "string") {
          ...
          setImagePreview((prevImage: any): VendorImage => ({ 
            ...prevImage, 
            image: result
          }));

          images[imagePreview.index] = result;
  ...
  const handleSubmit = (e: any) => {
    ...
    const product = {
      ...
      productImages: formData.productImages.map((file: any) => ({
        imageName: file.name,
                      ...
                      <Col lg={12}>
                        <div className="thumbs">
                          ...
                          images.map((image, imageIndex) => 
                            <div key={imageIndex} className="product-upload single" onClick={() => setImagePreview({
                              image, index: imageIndex
                            })}>
                              <div className={`avatar-upload`}>
                                <div className={`avatar-preview gi-preview ${imageIndex == imagePreview.index ? 'active' : ''}`}>
                                  <div className="imagePreview gi-div-preview">
                                    <img
                                      className="gi-image-preview"
                                      src={image}
